...
public unsafe class Canvas
{
    ...
    /// </summary>
    public uint Area => (uint)(_Width * _Height);
    ...
    public void DrawBlurredRectangle(int X, int Y, ushort Width, ushort Height, int Radial)
    {
        ...
        for (uint I = 0; I < Size; I++)
        {
            ...
            this[SX, SY] = new((byte)newAlpha[I], (byte)newRed[I], (byte)newGreen[I], (byte)newBlue[I]);
        }
    }
    ...
    /// <param name="Alpha">Whether to perform alpha blending with the background. Impacts performance.</param>
    ...
    public void DrawImage(int X, int Y, Canvas Image, bool Alpha = true)
    {
        ...
        if (!Alpha)
        {
            ...
            {
                Buffer.MemoryCopy(Image.Internal, Internal, Area * 4, Area * 4);
                return;
            }
        ...
        for (int IY = 0; IY < Image._Height; IY++)
        {
            ...
            if (IY < 0 || IY >= Image._Height || CanvasY < 0 || CanvasY >= _Height)
            {
                continue;
            }
            ...
            for (int IX = 0; IX < Image._Width; IX++)
            {
                ...
                // Skip if out of bounds.
                if (IX < 0 || IX >= Image._Width || CanvasX < 0 || CanvasX >= _Width)
                {
                    continue;
                }

                int CanvasIndex = (CanvasY * _Width) + CanvasX;

                // Retrieve background and foreground colors.
                uint BackgroundARGB = Internal[CanvasIndex];
                uint ForegroundARGB = Image.Internal[(IY * Image._Width) + IX];
                ...
                unchecked
                {
                    ...
                    // 	TODOTODO: support alpha bgs

                    // 	uint Color = 0xFFFF00FF;

                    // 	// byte A = (byte)(BackgroundA + (ForegroundA * (255 - BackgroundA) / 255));
                    // 	// byte R = (byte)((BackgroundR * BackgroundA / 255) + (ForegroundR * ForegroundA * (255 - BackgroundA) / (255*255)));
                    // 	// byte G = (byte)((BackgroundG * BackgroundA / 255) + (ForegroundG * ForegroundA * (255 - BackgroundA) / (255*255)));
                    // 	// byte B = (byte)((BackgroundB * BackgroundA / 255) + (ForegroundB * ForegroundA * (255 - BackgroundA) / (255*255)));
    ...
    {
        MemoryOperations.Fill(Internal, Color.ARGB, (int)Area);
    }
    ...
    {
        Buffer.MemoryCopy(Internal, Destination, Area * 4, Area * 4);
    }
    ...
    /// <summary>
    /// The graphics frame buffer, with a size matching <see cref="Area"/>.
    /// </summary>
